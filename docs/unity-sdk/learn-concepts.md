> Source: https://docs.dev.board.fun/learn/concepts
...
Any physical object on the display detected by the touch sensor. All contacts have an ID, position, and phase. Piece contacts additionally include orientation and touched status. There's no artificial limit on simultaneous contacts, only what physically fits on the display.
...
The process of maintaining contact identity across frames. Each contact has a persistent ID for the duration of its lifecycle.
...
Board filters out unintentional contact such as palms, arms, or elbows resting on the display. This happens automatically.
