> Source: https://docs.dev.board.fun/unity/getting-started/deploy
...
- Package Name (Player Settings > Other Settings): this is the package you'll pass to `board-connect launch <package>`.
...
1. `board-connect pair <address>` — run once; the user taps Approve on the device.
...
| Pairing fails or is rejected | Re-run `board-connect pair <address>` (pass `--force` to mint a fresh token) and have the user tap Approve on the device. The Board's address is shown under Settings > System. |
